...
def table_create(request):
    if request.method == 'POST':
        form = TableForm(request.POST, initial={'user': request.user})
        if form.is_valid():
        ...
        form = TableForm(initial={'user': request.user})
...
def reservation_create(request):
    if request.method == 'POST':
        ...
        if not table_number or not hour or not date:
        ...
        table = get_object_or_404(Table, table_number=table_number, user=request.user)
